...
from tensor import net_grasp_align as net
...
class Grasp_AlignCOM(Common):
    ...
    def __init__(self,train=False):
        ...
        if(not train):
            self.net = net.NetGrasp_Align()
            self.sess =self.net.load(var_path=self.Options.policies_dir+'grasp_net_10-20-2016_16h41m56s.ckpt')
    ...
    def execute_motion(self,yumi,theta=90.0): #was execute_mot not execute_motion
        ...
        #go to fixed position and fixed orientation
        fixed_pos = YuMiState([110,-98,-8,-66,71,160,-58])
        yumi.right.goto_state(fixed_pos)
        #open gripper
